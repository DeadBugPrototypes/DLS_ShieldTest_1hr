...
void SetAlarmHr(uint8_t AntallHr)
{
  Wire.begin(DS1337_ADDRESS);
  Wire.beginTransmission(DS1337_ADDRESS);
  Wire.write((byte) 0);	
  ...
  Wire.read(); // hopper over minutter
  uint8_t hr = bcd2bin(Wire.read());
  ...
  Wire.write(0b00000000);      // Alarm 1 minutter
  Wire.write(bin2bcd(hr));      // Alarm 1 timer (flagg satt)
